    ...
    <div className="pb-4">
      ...
      <div className="mt-4 space-y-3">
        {areas.map((area, idx) => {
          ...
          return (
            <label key={idx} className="flex justify-between items-center cursor-pointer select-none">
              <div className="flex items-center gap-2">
                ...
                <label htmlFor={`area-${idx}`} className="text-base text-grayColor1">{area.label}</label>
              </div>
              <span className="text-base text-grayColor1">{area.count}</span>
            </label>
          );
        })}
      </div>
