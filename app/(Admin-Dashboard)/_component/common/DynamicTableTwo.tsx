          ...
          <thead className="bg-neutral-50">
            <tr>
              {columns.map((col, index) => (
                <th key={index}  style={{ width: col.width || "auto" }} className="px-4 py-3 whitespace-nowrap text-sm font-medium text-[#4a4c56]">
                  {col.label}
                </th>
              ))}
              {(onView || onDelete) && (
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map((row, i) => (
                <tr key={i} className="border-t">
                  {columns.map((col, idx) => (
                    <td key={idx}  style={{ width: col.width || "auto" }} className="px-4  py-3 text-xs text-[#777980]">
