...
import { statusStyles } from '../dashboard/StatusBadge';
...
export default function Usermodal({ title = 'User Details', onClose, userData  }: UserModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={handleBackdropClick}>
      <div className="p-4 md:p-6 bg-white rounded-xl flex flex-col gap-4 md:gap-6 max-h-[90vh] overflow-auto w-full max-w-[974px] m-4">
        <div className="flex justify-between items-start">
          <h2 className="text-[#070707] text-xl md:text-[28px] font-medium">{title}</h2>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="w-full md:w-[300px] lg:w-[349px] p-4 rounded-2xl outline outline-[#f3f3f4] flex flex-col gap-6">
            ...
            <div className="flex justify-between items-center mb-4">
             {
              userData.role == "host" ? <div className="flex items-center gap-2">
              ...
              <span className="text-[#4a4c56] flex items-center gap-1 text-base"><FaStar className='text-yellow-400'/> {userData.verificationStatus.rating}</span>
            </div>
          ...
          <div className="flex-1 bg-bgColor rounded-xl p-4">
            ...
            <div className="flex  md:flex-row gap-4 md:gap-6">
              ...
              <div className="flex-1 flex flex-col gap-3 md:gap-5">
                {[userData.details.joinDate, userData.details.gender, userData.details.dateOfBirth, userData.details.language, userData.details.lastActive].map((text, i) => (
                  <div key={i} className="text-[#070707] text-base md:text-lg">{text}</div>
                ))}
              </div>
            </div>
            {
              userData.role == "Host" && <div className=' flex flex-wrap  gap-3 justify-center  mt-3 py-1.5 rounded-md border border-blackColor/20'>
