...
export default function PaymentPage() {
  ...
  const [payment, setPayment] = useState<"all" | "PayPal" | "Credit Card" | "Stripe">("all");
  const [selectedData, setSelectedData] = useState<any | null>(null);
  const [selectedRole, setSelectedRole] = useState<"All" | "Booking" | "Refunds" >("All");
  const [dateRange, setDateRange] = useState<"all" | "7" | "15" | "30">(
    "all"
  );
  ...
  const columns = [
    ...
    },
    {
      label: "Action",
      accessor: "status",
      formatter: (_, row) => <PaymentAction  onView={handleViewDetails} status={row} />,
    },
  ];
  const Bookcolumns = [
 ...
 const stats= [
  ...
  const filteredUsers = listingData.filter((user) => {
    const roleMatch = selectedRole === "All" || user.transactionType === selectedRole || user?.pymentMethod == selectedRole;
  ...
  value={payment}
  onValueChange={(value) =>
    setPayment(value as "all" | "PayPal" | "Credit Card" | "Stripe")
  }
>
  ...
  <SelectContent>
    ...
    <SelectItem value="Stripe">Stripe</SelectItem>
          ...
          <div className=" items-center flex gap-1  md:gap-2 text-sm ">
            ...
            >
              ...
              width={14}
              height={14}
            /><SelectValue placeholder="All Time" className="text-[#0068ef]" />
  </SelectTrigger>
                <SelectContent>
    ...
    <SelectItem value="Stripe">Last 30 days</SelectItem>
        ...
        <div>
      {
        selectedRole === "All" &&<DynamicTableWithPagination
        columns={columns}
        data={filteredUsers}
      ...
      {
        selectedRole === "Booking"  &&<DynamicTableWithPagination
        columns={Bookcolumns}
        data={filteredUsers}
        currentPage={currentPage}
        itemsPerPage={10}
      ...
      {
        selectedRole === "Refunds"  &&<DynamicTableWithPagination
