...
export default function PropertyPhotosPage() {
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (files.length < 5 && listProperty?.photos?.length < 5) {
  ...
  useEffect(() => {
    setPreviewImages(listProperty?.photos?.map(file => URL.createObjectURL(file)) || []);
  }, [])
